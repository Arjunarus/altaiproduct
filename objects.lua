...
obj {
    ...
    tak = function()
        if triggers.mainTask then
            p 'Пожалуй, теперь мне он может понадобиться. Я отсоединяю кабель и беру в руки аппарат.'
        else
            p(rndItem({
...
obj {
    ...
    use = function(this, that)
        ...
        elseif that^'МиниАТС' then
            p 'Кабель мне самому пригодится.'
        else
...
obj {
    ...
    verb = 'позвонить';
    inv = 'Телефон с подсоединенным к нему кабелем. Теперь его можно подключить к телефонной розетке.';
};
...
obj {
    ...
    act = function(self)
        local messages = {
            roof_0 = 'Очевидно, это телефонный кабель, это его я видел у входа на склад.';
            roof_1 = 'Черный кабель пролегает через всю крышу.';
            roof_2 = 'Провод выходит из щели, рядом с дверью.';
            stock_point = 'Довольно толстый двужильный кабель, такой, наверное, и 220 бы спокойно выдержал.';
            stock_enter = triggers.seenStockLine and 'Теперь мне ясно, что это телефонный кабель связывающий склад и офис.' or 'Интересно, что это за провод, отсюда и не поймешь...';
        }
        if here().nam ~= 'stock_enter' then
            triggers.seenStockLine = true
        
        elseif here().nam == 'serv' then
            -- TODO нужно вытащить кабель из АТС подключить туда телефон и проверить что порт АТС исправен, не забыть вставить кабель обратно
        end
